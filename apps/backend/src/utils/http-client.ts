  ...
  try {
    ...
    if (originalContentType?.includes("application/x-www-form-urlencoded")) {
      ...
      try {
        ...
        Object.entries(body as Record<string, string>).forEach(
          ([key, value]) => {
...
export async function forwardRequest(
  ...
  try {
    ...
    if (method !== "GET" && method !== "HEAD" && body !== undefined) {
      try {
        const { body: preparedBody, headers: updatedHeaders } =
          prepareRequestBody(body, originalContentType, filteredHeaders);
