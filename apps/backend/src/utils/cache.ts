...
export async function checkCacheAndServe(
  ...
  try {
    ...
    try {
      ...
      fastify.log.warn(
        "Failed to generate cache key during cache check:",
        error
      );
    ...
    try {
      ...
      try {
        if (fastify.snapshotManager) {
          // TODO: Implement updateAccess method in SnapshotManager
      ...
      try {
        ...
        reply.header(
          "X-Cache-Age",
          Math.floor((Date.now() - cached.createdAt) / 1000).toString()
        );
...
export async function storeInCache(
  ...
  try {
    ...
    try {
      ...
      fastify.log.warn(
        "Failed to generate cache key during cache store:",
        error
      );
    ...
    try {
      if (fastify.snapshotManager) {
        ...
        // Record the snapshot
        // TODO: Implement recordSnapshot method in SnapshotManager
